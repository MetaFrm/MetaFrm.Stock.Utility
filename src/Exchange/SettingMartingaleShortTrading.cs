...
namespace MetaFrm.Stock.Exchange
{
    ...
    /// </summary>
    public class SettingMartingaleShortTrading : Setting, ISettingAction
    {
        /// <summary>
        /// GapRate
        /// </summary>
        ...
        public new void Run(Models.Order? allOrder)
        {
            try
            {
                ...
                foreach (WorkData dataRow in this.WorkDataList)
                {
                    ...
                    var askOrder = allOrderList.SingleOrDefault(x => x.Side == "ask" && x.Price == dataRow.AskPrice && x.Volume == dataRow.AskQty);
                    if (askOrder != null)
                    {
                ...
                workDataList = this.WorkDataList.Where(x => x.BidOrder != null && x.BidOrder.UUID != null && x.BidOrder.UUID != "" && x.BidOrder.State == "wait"
                                                            && (x.AskOrder == null || x.AskOrder.UUID == "" || x.AskOrder.State == ""));
                foreach (var item in workDataList)
                {
                    item.AskOrder = new()
                    {
                ...
                if (workDataList.Count() == 1)
                {
                    try
                    {
                        ...
                        this.Organized(this.SettingID, false, true, false, this.IsProfitStop);

                        //자동전환 일때 원상 복구
                        //if (setting.IsOutChange && setting.OutPrice != 0 && setting.OutQty != 0 && setting.LastTradePrice > (setting.OutPrice * 1.01M))
                        //{
                        //    System.Threading.Thread.Sleep(60);
                        //    lock (setting.User.LockSettingList)
                        //        order1 = setting.Api.MakeOrder(setting.Market, OrderSide.ask, setting.OutQty, setting.OutPrice);
                ...
                {
                    this.Organized(this.SettingID, false, true, false, true);
                    this.WorkDataList = null;
                    return;
                }
                ...
                if (workDataList.Any())
                {
                    var maxAskPrice = this.WorkDataList.Where(x => x.AskOrder != null && x.AskOrder.UUID != null && x.AskOrder.UUID != "" && x.AskOrder.State == "done"
                                                        && (x.BidOrder == null || x.BidOrder.UUID == null || x.BidOrder.UUID == "")).Max(y => y.AskPrice);
                ...
                if (!this.FirstFix)
                {
                    ...
                    if (bidMinPrice > this.CurrentInfo.TradePrice)
                    {
                        ...
                        if (!workDataList.Any())
                        {
                            ...
                            if (!workDataList.Any())
                            {
                                this.Organized(this.SettingID, false, true, false, false);
                                this.WorkDataList = null;
                                return;
                            }
                        }
        ...
        private List<WorkData>? GetWorkData(decimal? TRADE_PRICE)
        {
            ...
            data["1"].CommandText = "[dbo].[USP_WORK_DATA_MARTINGALE_S_TRADING]";
